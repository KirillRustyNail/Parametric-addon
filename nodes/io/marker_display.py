...
MARKER_TYPES = [
    ...
    ('CIRCLE', "Circle", "Display marker as circle", 'VIEW3D_CURSOR', 3), # Похоже на круг
    ('CUBE', "Cube", "Display marker as cube", 'MESH_CUBE', 4),
    ('SPHERE', "Sphere", "Display marker as sphere (Icosphere)", 'MESH_ICOSPHERE', 5),
    # ('CONE', "Cone", "Display marker as cone", 'MESH_CONE', 6), # Пока без конуса
...
class MarkerDisplayNode(CadQueryNode):
    ...
    def process(self):
        ...
        try:
            ...
            if selector_in is None: logger.debug(f"Node {self.name}: No selector data provided."); return # Не ошибка, просто нет селекторов
        except Exception as e:
            raise NodeProcessingError(self, f"Input error: {e}")

        # 4. Получить Shape (если нужно, пока не используем, т.к. селекторы - объекты CQ)
        # current_shape = None
        # if isinstance(obj_in, cq.Workplane): ...
        # elif isinstance(obj_in, cq.Shape): current_shape = obj_in
        # else: raise NodeProcessingError(f"Unsupported Geometry type: {type(obj_in)}")
        ...
        else: logger.warning(f"Node {self.name}: Invalid selector input type: {type(selector_in)}. Expected Vertex, Edge, Face or list."); return
        ...
        for i, sel in enumerate(selectors):
            ...
            if isinstance(sel, cq.Vertex):
                 ...
                 try: points_to_mark.extend([v.toTuple() for v in sel.Vertices()])
                 except: logger.warning(f"Could not get coordinates for Face selector {i}")
            ...
            for point_idx, loc in enumerate(points_to_mark):
                 try:
                      ...
                      marker = None
                      is_mesh_marker = False

                      if marker_type in {'CUBE', 'SPHERE'}:
                           is_mesh_marker = True
                           # --- Создаем меш-примитив и настраиваем материал ---
                           current_active = context.view_layer.objects.active # Запоминаем активный объект
                           bpy.ops.object.select_all(action='DESELECT') # Снимаем выделение со всего

                           if marker_type == 'CUBE':
                               bpy.ops.mesh.primitive_cube_add(size=marker_size, location=loc, scale=(1,1,1))
                           else: # SPHERE (Icosphere)
                               bpy.ops.mesh.primitive_ico_sphere_add(radius=marker_size/2.0, subdivisions=2, location=loc, scale=(1,1,1)) # subdivisions=2 для гладкости

                           marker = context.view_layer.objects.active # Новый объект стал активным
                           if not marker: raise RuntimeError("Failed to get created mesh primitive.")
                           marker.name = marker_name

                           # Создаем или получаем материал
                           # Используем RGBA цвет из свойств ноды
                           mat_rgba = marker_color  # marker_color_ - это FloatVectorProperty(size=4)
                           mat_name = f"CQMarkerMat_{mat_rgba[0]:.2f}_{mat_rgba[1]:.2f}_{mat_rgba[2]:.2f}_{mat_rgba[3]:.2f}" # Включаем альфу в имя
                           ...
                           if not marker_mat:
                                ...
                                # Очищаем стандартные ноды, если они есть (не обязательно)
                                # if marker_mat.node_tree: marker_mat.node_tree.nodes.clear()
                           ...
                           # Восстанавливаем выделение и активный объект (важно!)
                           bpy.context.view_layer.objects.active = current_active
                           # marker.select_set(False) # Снимаем выделение с маркера
                           # --------------------------------------------------
                      else:
                           # --- Создаем Empty (как и раньше) ---
                           is_mesh_marker = False
                           marker = bpy.data.objects.new(marker_name, None)
                           marker.location = loc
                           marker.empty_display_size = marker_size
                           marker.empty_display_type = marker_type
                           # --- Устанавливаем цвет ТОЛЬКО для Empty ---
                           marker.color = marker_color
                           collection.objects.link(marker)
                           # -----------------------------------------

                      # Общие настройки маркера (имя уже установлено)
                      # Добавляем имя в нашу коллекцию для последующей очистки
                      item = self.marker_names.add(); item.name = marker.name
                      created_count += 1

                 except Exception as e_create:
                      logger.error(f"Failed to create marker at {loc} for selector {i}: {e_create}", exc_info=True) # exc_info для деталей
